...
function TestPage(props: any) {
  const text1 = ["당신은", "당신은"];
  const text2 = [
    "먼 곳보다 가까운 이웃들에게 도움을 주는 것에 더 관심이 있고",
    "지구 전역의 불우한 이웃들에게 관심이 많고",
  ];
  const text3 = [
    "특정 소수보다는 다수가 골고루 혜택을 받는 것을 선호하며,",
    "한 사람과 좀 더 깊은 관계를 가지는 것을 선호하며,",
  ];
  const text4 = [
    "모임이나 단체에 가입하여 참여하는 것에 부담을 느낄 수 있습니다.",
    "어딘가에 소속되어 일하는 것에서 즐거움을 느낄 수 있습니다.",
  ];
  const text5 = [
    "또,",
    "또, 일회성으로 후원하는 것보다 직접 시간과 마음을 쏟으며 남을 돕는 일에 보람을 느끼고",
  ];
  const text6 = [
    "사회적 가치가 담긴 제품에 별 관심이 없으며",
    "같은 제품이라도 사회적 가치가 담겨있는 제품을 더 선호하는 성향이 있으며",
  ];
  const text7 = [
    "후원과 종교는 별 관련이 없다고 생각합니다.",
    "종교는 더 나은 사회를 만든다고 믿습니다.",
  ];
  const text8 = [
    "그리고 규모와 전통이 있어 좀 더 대중적이며 믿을 수 있는 후원단체를 선호합니다.",
    "그리고 새로운 방식의 기부나, 새로 생겨나는 후원단체에 관심이 있습니다.",
  ];
  ...
  const getText = () => {
    return `${text1[testList[0].value]} ${text2[testList[1].value]} ${
      text3[testList[2].value]
    } ${text4[testList[3].value]} 
    ${text5[testList[4].value]} ${text6[testList[5].value]} ${
      text7[testList[6].value]
    } ${text8[testList[7].value]}`;
  };
  ...
  const getTestPage = () => {
    ...
    axios
      .get("https://server.ibingo.link/testpage", {
  ...
  const yesBtn = (index: number) => {
    testList[index].value === 0
      ? dispatch(changeTestValue(index, 1))
      : dispatch(changeTestValue(index, 0));
  ...
  const handleContentClick = (ngoId: number) => {
    ...
    <div id='testPageContainer'>
      ...
      <div id='resultPart' style={{ opacity: resultVisible }}>
        <div id='textPart'>
          ...
          <div id='cardResult'>
            {selectedOptions.map((el: number, i: number) => (
              <span key={i} id='selectedCard'>
                {optionList[el].name}
              </span>
            ))}
          </div>
        ...
        <div id='ngoPart'>
          ...
          <Link
            ...
            style={{ opacity: resultVisible }}
            to='/content'
          >
            <button id='moreInfoBtn'>이 단체 더 알아보기</button>
          </Link>
      ...
      {testList.map((test, i) => {
        if (i === 0) {
          return (
            <Motion
              key={i}
              style={{ left: spring(test.left), opacity: spring(test.opacity) }}
            >
              {({ left, opacity }) => (
                <div
                  id='testMainPart1'
                  ...
                  <div id='testOptions'>
                    {optionList.map((option, i) => (
                      <button
                        key={i}
                        id='optionBtn'
                        onClick={() => handleOptionBtnClick(i)}
            ...
            >
              {({ left, opacity }) => (
                ...
                >
                  ...
                  <div id='testMainQuestion'>{test.question}</div>
                  <div id='testMainBtnBox'>
                    <button id='testMainYesBtn' onClick={() => yesBtn(i)}>
                      네
                    </button>
                    <button id='testMainNoBtn' onClick={() => noBtn(i)}>
                      아니오
                    </button>
                  </div>
                </div>
      ...
      <div id='toastMessage' style={{ display: toast.visible, zIndex: 200 }}>
        {toast.message}
      </div>
